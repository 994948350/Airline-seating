...
BPlusTree<int> b4=b1;
BPlusTree<int> b5(4);
b5.insert(10000000000000000000);
b5=b1;
outfile<<"----------------------same----------------------------"<<endl;
b1.print_BFS_pretty(outfile);
outfile<<endl;
b2.print_BFS_pretty(outfile);
outfile<<endl;
b4.print_BFS_pretty(outfile);
outfile<<endl;
b5.print_BFS_pretty(outfile);
outfile<<endl;
outfile<<"----------------------different----------------------------"<<endl;
b3.print_BFS_pretty(outfile);
}

void test2(std::ofstream& outfile){
	outfile<<"=====test2 begin"<<endl;
	BPlusTree<int> b1(4);
	for(int i=20;i>0;i--){
		b1.insert(i);
	}
	outfile<<"=====test2 bfs"<<endl;
	b1.print_BFS(outfile);
	outfile<<"=====test2 sw"<<endl;
	b1.print_sideways(outfile);
	outfile<<"=====test2 complete fix pretty!!!"<<endl;
	b1.print_BFS_pretty(outfile);
}
