...
int arithmetic_operations() {
  ...
  int f = 2*b + 2*c;          
  int g = e - (b/c) + d + 20; 
  int h = round(((float)f/(float)c)/(float)a);  
  int m = (d / h) / 7;              
  int n = g + m;                   
  int p = -ceil((float)f/(float)e -(float)h); 
  int q = f + 2*d;                  
  int r = 2*g + m + p + n;           
  float s = (float)a / (float)f;  
...
int array_operations() {
  ...
  int** tmp_ptr = array;tmp_ptr++;
  for(int x = 1; x < size; ++x, ++tmp_ptr) {
    int* tmp_ptr2 = *tmp_ptr;++tmp_ptr2;
...
bool vector_compare(const std::vector<int> v1, const std::vector<int> v2) {
  bool success = true;  int small_size=v1.size()< v2.size()?v1.size(): v2.size();//added
 for(uint i=0; i<small_size; ++i){
    if(v1[i] < v2[i]) {
      success = false;
    }
  }
...
int main(int argc, char* argv[]) {
  ...
  if(ops == "--all-operations") { // only do it if testing everything
    if(decrypt(records, file_buffer, file_buffer_length, outFileName)) {
      std::cout << std::endl << "Decryption successful - good job!" << std::endl << std::endl;delete []file_buffer;
      return 0;
    } else {
      std::cout << "Decryption failed or was incomplete" << std::endl;delete []file_buffer;
      return 1;
    }
  }
